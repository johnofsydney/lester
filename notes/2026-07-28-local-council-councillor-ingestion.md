...
- **`Councils::Nsw::ImportCouncilResultRowJob`** — fetches one council's results page, parses declared-elected candidates (name + party where shown), resolves each `Person` via `People::RecordPerson`, resolves the council `Group` via `Groups::RecordGroup` (tagging it into `local_councils_tag` if newly created), and calls the fixed `Group::RecordRow` with `start_date:` = the declared date and `evidence:` citing the NSW Electoral Commission + election year. Also closes out anyone previously recorded as an open member of that council who isn't among the newly-declared winners — `end_date:` = the new term's declared date, `evidence:` citing "not returned in the \<year\> NSW LG election."
- **`Councils::Vic::IngestElectionResultsJob`** / **`Councils::Vic::ImportCouncilResultRowJob`** — mirrors the NSW pair, minus party-affiliation capture (not on the VIC page).

  ...
  class << self
    ...
    def affiliations_upload(file)
      ...
      csv.each do |row|
        ...
        owning_group.update(business_number: row['business_number']abn.gsub(/\D/, '')) if row['business_number'].present?
        member_group.update(business_number: row['abn']abn.gsub(/\D/, '')) if row['abn'].present?

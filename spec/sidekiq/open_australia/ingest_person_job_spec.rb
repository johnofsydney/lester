...
RSpec.describe OpenAustralia::IngestPersonJob, type: :job do
  ...
  context 'when ingest raises OpenAustraliaRateLimitError (transient)' do
    ...
    it 're-raises (so Sidekiq retries) and records an ingest failure' do
      expect { perform }.to raise_error(OpenAustraliaRateLimitError).and change(IngestSourceStatus, :count).by(1)
    end
  end
  ...
  context 'when ingest raises a non-rate-limit OpenAustraliaApiError (deterministic, not retryable)' do
    ...
    it 'does not re-raise, but records an ingest failure' do
      expect { perform }.not_to raise_error
      expect(IngestSourceStatus.count).to eq(1)
    end
  end
end

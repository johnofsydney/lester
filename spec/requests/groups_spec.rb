require 'rails_helper'

RSpec.describe 'Groups index pagination' do
  describe 'GET /groups' do
    before do
      30.times { |n| create(:group, name: format('Group %02d', n)) }
    end

    it 'shows the first page of results at the default page size' do
      get '/groups'

      expect(response).to have_http_status(:ok)
      expect(response.body.scan(/Group \d\d/).uniq.size).to eq(25)
    end

    it 'shows the remaining results on page 2 without repeating or skipping rows' do
      get '/groups'
      page_one_names = response.body.scan(/Group \d\d/).uniq

      get '/groups', params: { page: 2 }
      page_two_names = response.body.scan(/Group \d\d/).uniq

      expect(page_two_names.size).to eq(5)
      expect(page_one_names & page_two_names).to be_empty
      expect((page_one_names + page_two_names).uniq.size).to eq(30)
    end
  end
end
